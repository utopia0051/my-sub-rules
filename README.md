...
> **关于 Clash「全局模式」**：全局模式是客户端左上角的模式切换（规则/全局/直连），属客户端开关、不在本配置里。切到全局模式后所有流量走内置 GLOBAL 组（mihomo 自动包含全部节点），会绕过这里所有分流与防泄露规则，不建议日常用。想手动固定单个节点，保持规则模式、在「🔀机场or单个切换」里直接选即可——效果等同全局选节点，却不丢分流与防泄露。
